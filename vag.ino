...
void calibrationInstruction() {
  ...
  tft.print("500 grams");           // Sixth part of the text
...
void updateDisplay(long weight) {
  tft.fillRect(0, 80, 230, 60, 0x0000);  // Clear weight display area
  weight = (scale.get_units() * -1000);  // Get weight
  tft.setTextSize(3);                    // Set text size
  tft.setCursor(65, 30);                 // Set cursor position
  ...
  tft.setCursor(weightX, 80);                                     // Set cursor position
...
void displayCalibrationFactor() {
  if (!uiSwitched) {                // Check if not in calibration UI
    ...
    tft.setCursor(110, 140);        // Set cursor position
    tft.print(calibration_factor);  // Print calibration factor
  }
}
